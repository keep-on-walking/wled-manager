...
### Fixed
- **Boot Hang Issue**: Fixed wlan0-ip.service causing boot to hang
- Service now uses ConditionPathExists instead of wait loop
- Added 10-second timeout to prevent indefinite blocking
- Service gracefully skips if wlan0 doesn't exist (won't block boot)
- **Script Permissions**: Fixed usb-adapter-init.sh not being executable after installation

### Technical
- Removed ExecStartPre wait loop that could hang boot
- Added TimeoutStartSec=10 for safety
- Service only runs if wlan0 exists (ConditionPathExists)
- Install script now makes all scripts executable with chmod +x

## [1.2.0] - 2026-02-13

### Added
- **Offline Support**: Bootstrap CSS/JS now downloaded locally during installation
- Dashboard works without internet connection on client devices
- Automatic fallback to CDN if local files fail to load
- No internet required for devices connecting via WiFi AP

### Changed
- Installation script downloads Bootstrap files (~690 KB) for offline use
- Templates updated to prioritize local assets with CDN fallback
- Improved compatibility for devices without cellular data or WiFi internet

### Technical
- Bootstrap 5.3.0 CSS and JS bundled locally
- Bootstrap Icons included with fonts
- Smart fallback ensures compatibility if downloads fail during installation
