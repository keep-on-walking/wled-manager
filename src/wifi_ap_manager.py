...
class WiFiAPManager:
    ...
    def is_installed(self) -> bool:
        ...
        return os.path.exists(self.hostapd_conf) and os.path.exists('/usr/sbin/hostapd')
...
"""
            ...
            # Configure static IP on wlan0 using NetworkManager
            connection_name = "wlan0-ap"
            
            # Remove existing connection if it exists
            subprocess.run(
                ['/usr/bin/nmcli', 'connection', 'delete', connection_name],
                capture_output=True
            )
            ...
            if not success:
                logger.error("Failed to create NetworkManager connection")
                return False
